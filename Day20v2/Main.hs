...
foreign import ccall "mix.h mix" mix :: Ptr CInt -> CInt -> CInt -> Ptr CInt
...
main = do
    ...
    result2 <- fmap (sum . getCoords . map fromIntegral) $ peekArray len $ mix pCoords (fromIntegral len) 10
